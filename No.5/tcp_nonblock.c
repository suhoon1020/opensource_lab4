...
int main(int argc, char *argv[])
{
    ...
    {
        perror("Unable to put client sock intononblocking/async mode");
        exit(1);
    }
    for (;;)
        UseIdleTime();
...
void SocketHandler(int signalType)
{
    ...
    do
    {
        ...
        if ((recvMsgSize = recvfrom(sock, buf,
                                    MAXBUFSIZE, 0, (struct sockaddr *)&clnt_addr,
                                    &clntLen)) < 0)
        {
